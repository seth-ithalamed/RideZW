...
let splashImg: any = null;
try {
  splashImg = require('../assets/splash.png');
} catch {
  // Safe fallback if asset path differs
}
...
export function AuthGate({ role = 'driver', children }: { role?: 'rider' | 'driver'; children: ReactNode }) {
  ...
  const handleInitiateAuth = async () => {
    ...
    if (mode === 'signup') {
      ...
      if (role === 'driver') {
        ...
        if (!vehicleMake.trim()) {
          setError('Please enter your vehicle make and model (e.g. Toyota Passo).');
          return;
        }
        if (!vehiclePlate.trim()) {
          setError('Please enter your vehicle registration plate (e.g. AFE-8921).');
          return;
        }
      }
    }
    ...
    try {
      ...
      setError(e.message || 'Failed to dispatch verification SMS. Please verify your connection.');
    } finally {
      setSubmitting(false);
    }
  };
            ...
            <View style={s.otpSection}>
              ...
              >
                {submitting ? (
                  ...
                  <Text style={s.buttonText}>
                    {mode === 'signup' ? 'Verify & Complete Registration' : 'Verify & Enter Cockpit'}
                  </Text>
                )}
              </TouchableOpacity>
